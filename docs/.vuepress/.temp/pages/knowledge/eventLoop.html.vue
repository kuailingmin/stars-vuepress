...
<h3 id="执行优先级限制" tabindex="-1"><a class="header-anchor" href="#执行优先级限制"><span>执行优先级限制</span></a></h3>
<p>并不止一个消息队列，有异步队列和事件队列，而事件队列总是优先于异步队列被空闲下来的JS线程取用</p>
<ul>
<li>宏任务：script中代码，setTimeout，setInterval、I/O、UI render</li>
<li>微任务：Promise、Object.observe、MutationObserver</li>
</ul>
<font color=red  face="黑体">注:Object.observe好像被废弃了，有文章还说到Angular 2团队曾经实验性的使用了 O.o，但是因为性能原因最终放弃了</font><p><a href="https://github.com/luokuning/blogs/issues/1" target="_blank" rel="noopener noreferrer">点击查看具体原因</a></p>
<p>宏任务和微任务之间的关系图：
<img src="@source/images/event.png" alt=""></p>
<p>代码体现:</p>
<div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js"><pre v-pre><code class="language-javascript"><span class="line">  <span class="token keyword">new</span> <span class="token class-name">Promise</span><span class="token punctuation">(</span><span class="token punctuation">(</span><span class="token parameter">resolve</span><span class="token punctuation">)</span> <span class="token operator">=></span> <span class="token punctuation">{</span></span>
...
<ul>
<li>
<p>浏览器环境下的 异步任务 分为 宏任务(macroTask) 和 微任务(microTask)：</p>
</li>
<li>
<p>宏任务(macroTask)：script 中代码、setTimeout、setInterval、I/O、UI render；</p>
</li>
<li>
<p>微任务(microTask)： Promise、Object.observe、MutationObserver。</p>
</li>
<li>
<p>当满足执行条件时，宏任务(macroTask) 和 微任务(microTask) 会各自被放入对应的队列：宏队列(Macrotask Queue) 和 微队列(Microtask Queue) 中等待执行。</p>
</li>
</ul>
<h3 id="node-js环境" tabindex="-1"><a class="header-anchor" href="#node-js环境"><span>node.js环境</span></a></h3>
<p>在 Node 环境中 任务类型 相对就比浏览器环境下要复杂一些：</p>
<ul>
<li>
<p>microTask：微任务；</p>
</li>
<li>
<p>nextTick：process.nextTick；</p>
</li>
<li>
<p>timers：执行满足条件的 setTimeout 、setInterval 回调；</p>
</li>
<li>
<p>I/O callbacks：是否有已完成的 I/O 操作的回调函数，来自上一轮的 poll 残留；</p>
</li>
<li>
<p>poll：等待还没完成的 I/O 事件，会因 timers 和超时时间等结束等待；</p>
</li>
<li>
<p>check：执行 setImmediate 的回调；</p>
</li>
<li>
<p>close callbacks：关闭所有的 closing handles ，一些 onclose 事件；
idle/prepare 等等：可忽略。</p>
</li>
</ul>
<p>因此，也就产生了执行事件循环相应的任务队列 Timers Queue、I/O Queue、Check Queue 和 Close Queue。</p>
</div></template>
